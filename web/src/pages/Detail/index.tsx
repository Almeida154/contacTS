...
const Detail = () => {
  ...
  const [contactId, setContactId] = useState<string>(String(params.id));

  useEffect(() => {
    (async () => {
      setContactId(String(params.id));

      const contact: Contact = (await api.get(`contact/${contactId}`)).data;

      console.log(contact);

      const phoneNumbers = contact.telephones.map(phone => phone.number);
      const emailAddresses = contact.emails?.map(email => email.address);

      setFirstName(contact.firstName);
      setLastName(contact.lastName);
      contact.cpf && setCpf(contact.cpf);
  ...
  async function handleUpdateContact(e: FormEvent) {
    e.preventDefault();
    console.clear();
  ...
  return (
    <>
      <Container>
        ...
        <FormContainer>
          <span style={{ opacity: 0.5 }}>Veja mais detalhes</span>
